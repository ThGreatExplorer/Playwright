...
class Lexer(reader: java.io.Reader) {
   ...
   */
  def nextToken: Token = if (peek == -1) null
  else {
        ...
        val base: Int = radix match {
          ...
          case d if d.isDigit => {
            ...
            if (ending != 'r' && ending != 'R') {
              throw new UnexpectedCharException(ending, Position(_currentLine, _currentCol), "expected 'r' termination mark for radix")
            }
            r
          }
          case _ => {
            throw new UnexpectedCharException(radix, Position(_currentLine, _currentCol), "'#' should be followed by a radix")
          }
        }
...
object Lexer {
  ...
  class UnexpectedCharException(char: Char, position: Position, msg: String) extends Exception("Encountered unexpected character: '" + char + "' at " + position + ": " + msg)
