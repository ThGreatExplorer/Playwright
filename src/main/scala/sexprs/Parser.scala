...
object Parser {
  ...
  class EOFBeforeMatchingParenthesisException(startPos: Position) extends Exception("Opened parenthesis at position: " + startPos + " has no matching closing parenthesis")
  class UnexpectedTokenException(token: Token, pos: Position)     extends Exception("Unexpected token: " + token + " at position: " + pos)
